...
def data():
	if request.method == "POST":
		...
		m = list(records.find({"title" : concept_user_likes }))
		if len(m)==0:
			return render_template('error.html',msg='Invalid data,Go back and enter again')
		else:
			cv = CountVectorizer()
			count_matrix = cv.fit_transform(df["combined_features"])
			cosine_sim = cosine_similarity(count_matrix) 
			movie_index = get_index_from_title(concept_user_likes)
			if movie_index==max_val:
			...
			for element in sorted_similar_concepts:
				if concept_user_likes!=get_title_from_index(element[0]):
					k.append(get_title_from_index(element[0]))
			l = []
			i=0
			for ele in k:
				l.append(records.find_one({'title':ele}))
				i=i+1
				if i>2:
					break
